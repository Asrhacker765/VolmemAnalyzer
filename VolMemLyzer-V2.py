...
# Define the VOL_MODULES dictionary with explicit plugin names for ambiguous ones
VOL_MODULES = {
    ...
    'mftscan.ADS': extract_mftscan_features,  # Specify exact plugin
...
def invoke_volatility3(vol_py_path, memdump_path, module, output_to):
    vol_script = os.path.join(vol_py_path, 'vol.py')
    if not os.path.exists(vol_script):
        raise FileNotFoundError(f"Volatility script not found at {vol_script}")
    with open(output_to, 'w') as f:
        command = ['python3', vol_script, '-f', memdump_path, '-r', 'json', 'windows.' + module]
    ...
    with tempfile.TemporaryDirectory() as workdir:
        ...
        for module, extractor in VOL_MODULES.items():
            ...
            try:
                ...
                print(f"Error executing module {module}: {e}")
            except KeyError as e:
                print(f"KeyError for module {module}: {e}")
            except Exception as e:
                print(f"Unexpected error for module {module}: {e}")
